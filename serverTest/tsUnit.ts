module tsUnit {
    ...
    export class Test {
        ...
        private testResult = new TestResult();
                    ...
                    if ( prop.match("test_*") ){
                        if (typeof testClass[prop] === 'function') {
                                ...
                                try {
                                    ...
                                    var myProp = prop;
                                    testClass[prop]( 
                                    function ( err: Error, testMethod:string ) {
        ...
        private showResults( target: HTMLElement) {
            var template = '<article>' +
                    '<h1>' + this.getTestResult( this.testResult ) + '</h1>' +
                    '<p>' + this.getTestSummary( this.testResult ) + '</p>' +
                    '<section id="tsFail">' +
                    '<h2>Errors</h2>' +
                    '<ul class="bad">' + this.getTestResultList( this.testResult.errors ) + '</ul>' +
                    '</section>' +
                    '<section id="tsOkay">' +
                    '<h2>Passing Tests</h2>' +
                    '<ul class="good">' + this.getTestResultList( this.testResult.passes ) + '</ul>' +
                    '</section>' +
                '</article>';
