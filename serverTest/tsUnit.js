...
(function (tsUnit) {
    var Test = (function () {
        ...
        Test.prototype.run = function (target) {
                    ...
                    if(prop.match("test_*")) {
                        if(typeof testClass[prop] === 'function') {
                            try  {
                                var that = this;
                                var myProp = prop;
                                testClass[prop](function (err, testMethod) {
                                    that.testResult.errors.push(new TestDescription(testName, testMethod, err.message));
                                    that.showResults(target);
                                }, function (testMethod) {
                                    that.testResult.passes.push(new TestDescription(testName, testMethod, 'OK'));
                                    that.showResults(target);
                                });
                            } catch (err) {
                                this.testResult.errors.push(new TestDescription(testName, prop, err.message));
                                this.showResults(target);
                            }
                        }
                    }
                }
        ...
        Test.prototype.showResults = function (target) {
            var template = '<article>' + '<h1>' + this.getTestResult(this.testResult) + '</h1>' + '<p>' + this.getTestSummary(this.testResult) + '</p>' + '<section id="tsFail">' + '<h2>Errors</h2>' + '<ul class="bad">' + this.getTestResultList(this.testResult.errors) + '</ul>' + '</section>' + '<section id="tsOkay">' + '<h2>Passing Tests</h2>' + '<ul class="good">' + this.getTestResultList(this.testResult.passes) + '</ul>' + '</section>' + '</article>';
    ...
    var TestContext = (function () {
        ...
        TestContext.prototype.notThrows = function (a) {
            try  {
                a();
            } catch (ex) {
                //isThrown = true;
